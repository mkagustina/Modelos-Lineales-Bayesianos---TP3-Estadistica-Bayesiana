...
$$ \ln(T(t) - T_{\mathrm{amb}}) = \ln( T_{\mathrm{diff}} \cdot e^{-rt}) = \ln(T_{\mathrm{diff}}) + (-rt) \cdot \ln (e) = $$
$$ = \ln(T_{\mathrm{diff}}) - rt = \beta_{0} + \beta_{1}\cdot t $$
donde $\beta_{0}$ sería el logaritmo de la diferencia entre la función $T$ y la temperatura ambiente cuando $t=0$ (si se exponencia a ambos lados se obtiene que $$ T(t=0) - T_{\mathrm{amb}} = e^{\beta_0}= e^{ln(T_{\mathrm{diff}})} = T_i - T_{\mathrm{amb}} \Longrightarrow T(0) = T_i$$) y $\beta_{1}$ es cuánto va cambiando el logaritmo de la diferencia entre la temperatura corporal y la temperatura del ambiente por cada hora que transcurre. Por este motivo es la constante de conductividad.
