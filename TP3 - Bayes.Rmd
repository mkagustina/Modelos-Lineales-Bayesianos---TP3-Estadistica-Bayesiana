...
$$ = -r \cdot [(T_i - T_{amb}) \cdot e^{-rt}] = $$
$$ = -r \cdot [-T_{amb}+T_{amb}+(T_i - T_{amb}) \cdot e^{-rt}] = $$
$$ = -r \cdot [-T_{amb}+T(t)] = $$
$$ = r \cdot [T_{amb} - T(t)] $$
...
Ahora bien, para mayor simplicidad a la hora de modelizar se decide trabajar con la diferencia entre la temperatura del cuerpo y la temperatura ambiente $T - T_{amb}$ y se define $T_{diff} = T_i - T_{amb}$; por lo que se obtiene finalmente que $T(t) - T_{amb} = T_{diff} \cdot e ^ {-rt}$. 
...
$$ ln(T(t) - T_{amb}) = ln( T_{diff} \cdot e^{-rt}) = ln(T_{diff}) + (-rt) = $$
$$ = ln(T_{diff}) - rt = \beta_{0} + \beta_{1}\cdot t $$
donde $$ \beta_{0} $$ sería el logaritmo de la diferencia entre la función $T$ y la temperatura ambiente cuando $t=0$ (si se exponencia a ambos lados se obtiene que $$ T(t=0) - T_{amb} = e^{\beta_0}= e^{ln(T_{diff})} = T_i - T_{amb} \Longrightarrow T(0) = T_i$$) y $$ \beta_{1} $$ es cuánto va cambiando el logaritmo de la diferencia entre la temperatura corporal y la temperatura del ambiente por cada hora que transcurre. Por este motivo es la constante de conductividad


Dado que $\beta_0$, $\beta_1$ y $\sigma$ serán las variables aletorias, las distribuciones a priori se definen sobre ellas. 
Para la de la ordenada al origen ($\beta_0$), se considerará una temperatura ambiente minima de -2° y una máxima de 33° junto con una temperatura inicial de 37° (temperatura habitual del cuerpo humano); esto daría que la diferencia máxima ocurre en el minimo de $T_{amb}$ y la minima se da en el máximo de $T_{amb}$. (min = 4, máx = 39) Como se está trabajando con $ln(T_i - T_{amb})$, el valor minimo será $ln(4) = 1.386$ y el máximo será $ln(39) = 3.664$. Dado este rango de valores, se deduce que $\mu_{\beta_0} = 2.525$ y $\sigma_{\beta_0} = 0.380$ y queda así definido su *prior*.
...
Ahora, $sigma$ es el desvio de la $T(t) - T_{amb}$ 
la diferencia máxima entre $T_{amb}$ y $T(t)$ se da cuando $T(t)$ sea máxima (37°) y $T_{amb}$ sea minima (-2°), dicha diferencia en 39; y la diferencia minima se da cuando la temperatura del cuerpo alcanza la temperatura ambiente, es decir la diferencia es 0. Por lo que el desvio de la distribucion de $\sigma$ es 6.5
