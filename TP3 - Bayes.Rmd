...
Para determinar la hora de muerte de Sergio, se trabajará con un modelo lineal bayesiano; se determinarán creencias a priori para los parámetros desconocidos para luego actualizarlas con las mediciones de temperatura que se realizaron al cuerpo del hombre. Para este proceso, se considera que la temperatura ambiente (constante a través del tiempo) es de 22ºC.
...
Se asignarán distribuciones a priori para los parámetros desconocidos $\beta_0$, $\beta_1$ y $\sigma$. Si bien $\mu$ también es desconocido, no se le asigna un prior propio ya que su valor depende exclusivamente de $\beta_0$ y $\beta_1$, de forma que una vez asignados valores para estos dos parámetros, $\mu$ queda definido. 

Como se menciona en la ecuación (2), $\beta_0$ es el $\ln(T_{\mathrm{diff}}) = \ln(T_i - T_{\mathrm{amb}})$. Se sabe que la temperatura ambiente es de 22ºC, mientras que la temperatura inicial del cuerpo se encuentra entre 36,5ºC y 37,5ºC (temperatura habitual del cuerpo humano). Con esto se puede decir que el valor mínimo para $\beta_0$ es $\ln(36,5^\circ-22^\circ)=\ln(14,5^\circ)=2,67^\circ$, y el máximo es $\ln(37,5^\circ-22^\circ)= \ln(15,5^\circ)=2,74^\circ$.
...
**CAMBIO:** el tiempo 0 es el momento de la primer medición. Sabemos que a las 5:33hs la vecina de Sergio escuchó ruidos provenientes de su casa, y la primer medición de temperatura se hizo a las 6:45hs. Entonces, podemos pensar que al momento de la primer medición Sergio llevaba muerto entre 1 y 2 horas. 
En ese tiempo, de esperar que la temperatura de su cuerpo haya descendido entre 2 y 6 grados, como mucho. Si consideramos la temperatura normal del cuerpo humano en 36.5ºC, entonces el $\ln(T(t) - T_{\mathrm{amb}})$ máximo es $\ln((36.5-2) - 22) = 2.526$ y el mínimo es $\ln((36.5-6) - 22) = 2.14$
Entonces podemos decir que $\beta_0$ tiene a priori una distribución normal con media $\mu_{\beta_0} = \frac{2.526 + 2.14}{2}=2.333$ y desvío $\sigma_{\beta_0} = \frac{2.526-2.333}{3} = 0.064$

Para definir una distribución para $\beta_1$, se puede utilizar la Figura 1; si bien el gráfico se realizó para una temperatura ambiente 1ºC más alta que la actual, las conclusiones son las mismas. Entonces, si un valor razonable para *r* se encuentra entre 0.1 y 0.2, se puede decir que $\beta_1 = -r$ toma valores entre -0.2 y -0.1.
...
**CAMBIO:** sin embargo, como Sergio murió desangrado y la pérdida de sangre puede acelerar el descenso de la temperatura corporal, se permitirá que *-r* tome valores entre -0.3 y -0.15. Por eso se propone a priori una distribución normal de media $\mu_{\beta_1} = \frac{-0.3-0.15}{2} = -0.225$ y desvío $\sigma_{\beta_1} = \frac{-0.15+0.225}{3} = 0.025$.

Por último, $sigma$ es el desvío del $\ln(T(t) - T_{\mathrm{amb}})$. La diferencia máxima entre la temperatura ambiente (22ºC) y $T(t)$ se da cuando $T(t)$ es máxima (37,5º). En ese caso, $\ln(37.5-22)=2.74$. La diferencia mínima es de cero grados, y se da cuando la temperatura del cuerpo alcanza la temperatura ambiente, pero ahí el logaritmo no existe. 
...
Se puede ver que el prior no es tan compatible con las observaciones. La pendiente parece estar bien, pero el $\beta_0$ es muy alto.
...
Lo único que cambia es la distribución de sigma.
Nuestro modelo actualizado con 1 obs es:
...
rectas_posterior <-
  ggplot(observaciones) +
  geom_point(aes(x=tiempo, y=y), size = 3) +
  geom_abline(data = dv1 %>%
  slice_sample(n = 50, weight_by = posterior), mapping = aes(slope=b1, intercept=b0), alpha = 0.3, size = 1) + 
   geom_point(data = observaciones %>% slice(1), aes(x=tiempo, y=y), size=4, color = "firebrick3") +
  lims ( x= c(-3,7.5),y=c(0.5,4)) +
  theme_bw() +
  geom_hline(yintercept = log(37-22), linetype = "dashed") +
...
Según este nuevo modelo, Serio murió entre el tiempo -0.3 y 0.3. Malísimo.
...
\mu = \beta_{0} + \beta_{1}\cdot t \\
\\
\beta_0 \sim \mathcal{N}_{(2.704, 0.012)} \\
\beta_1 \sim \mathcal{N}_{(-0.15, 0.017)} \\
\sigma \sim \mathcal{N}^{+}_{(0.33)}
\end{cases}
$$
...
rectas_posterior2 <-
  ...
  slice_sample(n = 50, weight_by = posterior), mapping = aes(slope=b1, intercept=b0), alpha = 0.3, size = 1) +
   geom_point(aes(x=tiempo, y=y), size = 3) +
  geom_point(data = observaciones %>% slice(1:2), aes(x=tiempo, y=y), size=4, color = "firebrick3") +
  lims ( x= c(-3,7.5),y=c(0.5,4)) +
  theme_bw()+
  geom_hline(yintercept = log(37-22), linetype = "dashed")  +
...
dv3 <-
  ...
  mutate(likelihood = purrr::pmap_dbl(list(datos = datos_transf3, b0 = parametros$b0, b1 = parametros$b1, sd = 0.6),
                                      calc_likelihood)) %>%
  mutate(posterior = likelihood*prior) %>%
  select(-datos_transf3) 

muestra3 <- dv3 %>% slice_sample(n = 50, weight_by = posterior)
...
generate_curve_data <- function(b0, b1, T_amb = 22, t_range = seq(-3.5, 8, by = 0.1)) {
  data.frame(
    t = t_range,
    T_t = T_amb + exp(b0 + b1 * t_range)
  )
}

# Crear un data frame con todas las curvas
curves_data <- muestra3 %>%
  group_by(b0, b1) %>%
  do(generate_curve_data(.$b0, .$b1))

# Graficar las observaciones y las curvas
rectas_posterior3 <-
  ggplot(datos3) +
  geom_line(data = curves_data, mapping = aes(x = t, y = T_t, group = interaction(b0, b1)), alpha = 0.3, size = 1) +
  # Ajustar el límite superior de y según sea necesario
  geom_point(aes(x = t, y = temp), size = 4, color = "firebrick3") +
  theme_bw()
  
rectas_posterior3 <-
  ggplot(observaciones) +
  geom_point(aes(x=tiempo, y=y), size=4, color = "firebrick3") +
  geom_line(data = muestra3, mapping = aes(y = exp(b0+b1*x)+22) +
  # geom_abline(data = muestra3, mapping = aes(slope=b1, intercept=b0), alpha = 0.3, size = 1) +
  lims ( x= c(-3.5,7.5),y=c(0.5,4)) +
  theme_bw()
# +
  # geom_hline(yintercept = log(36.5-22), linetype = "dashed")  +
  # geom_vline(xintercept = -3.16)+
...
## Cosas que no entiendo y me da cosa borrar
