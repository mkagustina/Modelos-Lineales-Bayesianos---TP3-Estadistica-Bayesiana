...
parametros <-
  expand.grid(b0 = seq(mu_b0-(3*sigma_b0), mu_b0+(3*sigma_b0),length.out = 100),
              b1 = seq(mu_b1-(3*sigma_b1), mu_b1+(3*sigma_b1),length.out = 100)) |>
...
rectas_prior <- 
  ...
  geom_point(aes(x=tiempo, y=y), size = 3, color = 'firebrick3') +
...
Evaluamos la calidad predictiva del nuevo modelo
...
rectas_prior <- 
  ...
  lims(x=c(-2,7),y=c(0.4,3)) +
  theme_bw() +
  labs(caption = 'Figura 3: rectas predictivas a posteriori con el modelo 1')
