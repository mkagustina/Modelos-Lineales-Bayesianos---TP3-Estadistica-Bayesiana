...
Se realizan curvas *a posteriori* con el modelo para poder dar una primera aproximación sobre la hora de muerte de Sergio.
...
rectas_posterior1 <-
  ...
  annotate("text", x = lim_inf1, y = 28, label = "x = 3.23", 
           color = "black", vjust = -0.5, angle = 90) +
  annotate("text", x = lim_sup1, y = 28, label = "x = 6.13", 
           color = "black", vjust = -0.5, angle = 90) +
...
Según la Figura 4, Sergio se habría muerto entre las 3:23hs y las 6:13hs.
...
Se cuenta entones con dos datos: la temperatura del cuerpo de Sergio a las 6:45hs, que fue de 32.8ºC y la temperatura del cuerpo de Sergio a las 8:15hs que fué de 30.5°C.
...
Nuevamente se obtienen buenos $\hat{R}$ para los 3 parámetros pero el número efectivo de muestras se ve disminuído en relación al del modelo 1.
...
rectas_posterior2 <-
  ...
  annotate("text", x = lim_inf2, y = 28, label = "x = 3.16", 
           color = "black", vjust = -0.5, angle = 90) +
  annotate("text", x = lim_sup2, y = 28, label = "x = 6.07", 
           color = "black", vjust = -0.5, angle = 90) +
...
\text{Parámetro} &\text{Media} &\text{desvío} & \hat{R} & \text{Número efectivo de muestras} \\
\hline
\beta_0 & 2.337 & 0.059 & 1.0000 & 4067 \\
\beta_1 & -0.238 & 0.022 & 1.0005 & 3409 \\
\sigma & 0.287 & 0.201 & 1.0016 & 2586 \\
\end{array}
$$
...
\mu = \beta_{0} + \beta_{1}\cdot t \\
\\
\beta_0 \sim \mathcal{N}_{(2.337, 0.059)} \\
\beta_1 \sim \mathcal{N}_{(-0.238, 0.022)} \\
\sigma \sim \mathcal{N}^{+}_{(0.201)}
\end{cases}
$$
...
rectas_posterior3 <-
  ...
  annotate("text", x = lim_inf3, y = 28, label = "x = 3.50", 
           color = "black", vjust = -0.5, angle = 90) +
  annotate("text", x = lim_sup3, y = 28, label = "x = 6.08", 
           color = "black", vjust = -0.5, angle = 90) +
...
En el caso de $\beta_0$ hay que considerar que la temperatura del cuerpo al momento de la muerte ya no es la habitual del cuerpo humano(36.5ºC), sino que ronda los 38 y 39ºC.
Luego, $\beta_1$ debería tener una media menor, para permitir que el descenso de la temperatura sea más rápido en las primeras horas postmortem.
